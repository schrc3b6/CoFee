...
public:
    ...
    static ErrorCheckedState getUnchecked()
    {
        return ErrorCheckedState(Unchecked);
    }
    ...
    : mallocFn("malloc"), callocFn("calloc")
{
    UseBeforCheckBugType.reset(
        new BugType(this, "Use variable before allocation check",
                                       ...
                                       CheckerContext &C) const
{
    ...
    if(!mallocFn.matches(Call) && !callocFn.matches(Call))
        return;
                                       ...
                                       CheckerContext &C) const
{
    ...
    do {
        ...
        if (sR) {
            SymbolRef sym = R->getSymbolicBase()->getSymbol();
...
void SimpleErrorChecker::checkPreCall(const CallEvent &Call,
                                      CheckerContext &C) const
{
    ProgramStateRef State = C.getState();
...
void SimpleErrorChecker::checkDeadSymbols(SymbolReaper &SR,
        CheckerContext &C) const
{
